...
class Solution {
    public int sumK(Node root, int k) {
        ...
        prefixSumMap.put(0, 1); // Base case to handle path sum exactly equal to k
        return countPaths(root, 0, k, prefixSumMap);
    }

    private int countPaths(Node node, int currSum, int k, HashMap<Integer, Integer> prefixSumMap) {
        ...
        count += countPaths(node.left, currSum, k, prefixSumMap);
        count += countPaths(node.right, currSum, k, prefixSumMap);
